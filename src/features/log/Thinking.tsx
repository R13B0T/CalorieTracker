...
const LINES = ['Squinting at it…', 'Counting the sneaky oil…', 'Consulting the food database…', 'Weighing things by eye…', 'Nearly there…'];
...
export function Thinking({ onCancel }: { onCancel?: () => void }) {
  ...
  return (
    <div className="card flex items-center gap-4" role="status" aria-live="polite">
      <QuokkaSprite stage={game?.pet.stage ?? 'joey'} mood="content" outfitId={game?.pet.outfitId} size={72} animate />
      <div className="flex-1">
        <div className="font-bold">{game?.pet.name ?? 'Quokka'} is thinking</div>
        <div className="text-sm text-bark-500">{LINES[i]}</div>
      </div>
      {onCancel && (
        <button className="btn-ghost px-3 py-1 text-sm" onClick={onCancel}>Cancel</button>
      )}
    </div>
  );
}
