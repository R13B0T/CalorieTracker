...
export function QuestsPanel() {
  ...
  const Row = ({ q }: { q: (typeof daily)[number] }) => {
    ...
    return (
      <li className={`flex items-center gap-3 rounded-xl px-3 py-2 ${done ? 'bg-euc-100' : 'bg-sand-100'}`}>
        <span className="text-xl" aria-hidden>{done ? '✅' : t.emoji}</span>
        <div className="flex-1 min-w-0">
          <div className={`text-sm font-bold ${done ? 'line-through text-bark-500' : ''}`}>{t.title}</div>
          {q.target > 1 && !done && (
        ...
        <div className="text-[10px] font-bold text-bark-500 text-right">
          ...
          <div>+{q.period === 'daily' ? XP.dailyQuest : XP.weeklyQuest} XP · {q.period === 'daily' ? COINS.dailyQuest : COINS.weeklyQuest}🪙</div>
        </div>
      </li>
    );
  };
  ...
  return (
    <div className="card flex flex-col gap-3">
      <div>
        ...
        {daily.length === 0 ? <p className="text-sm text-bark-500">Quests roll each morning. Log something to kick things off.</p> : <ul className="flex flex-col gap-1.5">{daily.map((q) => <Row key={q.id} q={q} />)}</ul>}
      </div>
      <div>
        <div className="font-black mb-2">This week</div>
        <ul className="flex flex-col gap-1.5">{weekly.map((q) => <Row key={q.id} q={q} />)}</ul>
      </div>
    </div>
  );
}
