...
export default function BodyPage() {
  ...
  const exercise = useLiveQuery(
    () => db.exercise.where('dayKey').equals(todayKey).sortBy('at'),
    [todayKey],
    [],
  );
  const fasts = useLiveQuery(
    () => db.fasting.orderBy('startedAt').reverse().limit(5).toArray(),
    [],
    [],
  );
  ...
  const rate =
    trend.length >= 8
      ? ((trend[trend.length - 1].trend - trend[trend.length - 8].trend) / 7) * 7
      : null;
  ...
  const daysUntilRecal = profile
    ? Math.max(
        0,
        Math.ceil(
          RECAL.minDays -
            (Date.now() - (profile.lastRecalibratedAt ?? profile.createdAt)) / 86_400_000,
        ),
      )
    : null;
    ...
    <div className="max-w-lg mx-auto px-4 pt-3 flex flex-col gap-4">
        ...
        <div className="card border-2 border-sun-500 flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={async () => {
                await applyRecalibration(suggestion);
                setSuggestion(null);
                toast('Target updated', 'reward', '📐');
              }}
            >
              Apply
            </button>
      ...
      <section className="card flex flex-col gap-3">
          ...
          <div className="flex items-end gap-2">
            <div className="w-28">
              <NumberField
                ...
                min={30}
                max={400}
                placeholder={latest ? latest.kg.toFixed(1) : '80.0'}
              />
            </div>
            <button className="btn-primary py-3" disabled={typeof kg !== 'number'} onClick={save}>
              Log
            </button>
          </div>
        </div>
        ...
        {profile && !suggestion && daysUntilRecal !== null && (
          <p className="text-xs text-bark-500">
            {daysUntilRecal > 0
              ? `Recalibration check in ${daysUntilRecal} day${daysUntilRecal === 1 ? '' : 's'}. Keep weighing in and logging.`
              : 'Recalibration needs 6+ weigh-ins and 70% of days logged in the window.'}
          </p>
        )}
        {weights.length > 0 && (
          <details className="text-sm">
            <summary className="cursor-pointer text-bark-700 font-semibold">
              Recent weigh-ins
            </summary>
                ...
                .map((w) => (
                  <li key={w.id} className="flex justify-between items-center">
                    <span className="text-bark-700">
                      {formatDayLabel(w.dayKey, todayKey)} · {formatTime(w.at)}
                    </span>
      ...
      <section className="card flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          <button className="btn-secondary py-2 text-sm" onClick={() => setExOpen(true)}>
            + Log
          </button>
        </div>
        {exercise.length === 0 ? (
        ...
        <p className="text-xs text-bark-500">
          {settings?.eatBackExercise
            ? 'Exercise is added to your food budget.'
            : 'Exercise is tracked but not added to your food budget (Settings to change).'}
        </p>
      </section>
      ...
      <section className="card flex flex-col gap-2">
        ...
        {fasts.length === 0 ? (
          <p className="text-sm text-bark-500">
            Start a fast from Today. Default window is {settings?.fastingDefaultHours ?? 16} hours.
          </p>
        ) : (
          <ul className="flex flex-col gap-1 text-sm">
            {fasts.map((f) => {
              ...
              return (
                <li key={f.id} className="flex justify-between">
                  <span className="text-bark-700">
                    {new Date(f.startedAt).toLocaleDateString('en-AU', {
                      day: 'numeric',
                      month: 'short',
                    })}{' '}
                    · {f.endedAt ? `${h.toFixed(1)} h` : 'in progress'}
                  </span>
                  <span className={`font-bold ${f.completed ? 'text-euc-700' : 'text-bark-500'}`}>
                    {f.endedAt
                      ? f.completed
                        ? 'Completed'
                        : 'Ended early'
                      : `${h.toFixed(1)} / ${f.targetHours} h`}
                  </span>
                </li>
              );
            })}
